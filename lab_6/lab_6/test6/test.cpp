...
int task2(char s1[81]) {
	...
	char gl[21] = "àèå¸îóûýþÿÀÈÅ¨ÎÓÛÝÞß";
	...
	for (int i = 0; i < w; i++) {
		...
		if (s[i] >= 'à' && s[i] <= 'ÿ' || s[i] >= 'À' && s[i] <= 'ß') sog++;
	}
	sog -= gl_;
	int ret;
	if (gl_ > sog) ret = 1;
...
TEST(TestTask2, Test1) {
	EXPECT_EQ(task2("öóöóöóö"), 0);
	EXPECT_TRUE(true);
}

TEST(TestTask2, Test2) {
	EXPECT_EQ(task2("öóöóóóóóöö"), 1);
	EXPECT_TRUE(true);
}

TEST(TestTask2, Test3) {
	EXPECT_EQ(task2("öóöó534534öö"), 0);
	EXPECT_TRUE(true);
}
...
TEST(TestTask3, Test1) {
	EXPECT_EQ(task3("ûâû"), 1);
	EXPECT_TRUE(true);
}

TEST(TestTask3, Test2) {
	EXPECT_EQ(task3("âàï4ïàâ"), 1);
	EXPECT_TRUE(true);
}

TEST(TestTask3, Test3) {
	EXPECT_EQ(task3("ûâêû"), 0);
	EXPECT_TRUE(true);
}
